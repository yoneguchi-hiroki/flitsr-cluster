...
class Flitsr(Ranker):
    ...
    def flitsr(self, spectrum: Spectrum, formula: str, use_cluster: bool = True) -> List[Spectrum.Group]:
        """
        Executes the recursive flitsr algorithm to identify faulty elements
        """
        #print("Running FLITSR on spectrum with", spectrum.tf, "failing tests")
        if (spectrum.tf == 0):
            return []
        ranking = self.run_metric(spectrum, formula)
        r_iter = iter(list(ranking))
        #print("r_iter",r_iter)
        entity = next(r_iter).entity
        prev_failing_count = spectrum.tf
        tests_removed = spectrum.get_tests(entity, only_failing=True,
                           remove=True, bucket='flitsr',add_failing_cluster=use_cluster)
        
        # Check if tests were actually removed from spectrum
        # If tests_removed has values but spectrum.tf didn't decrease, tests weren't actually removed
        while (len(tests_removed) == 0 or spectrum.tf == prev_failing_count):

            if ((s2 := next(r_iter, None)) is None):
                if use_cluster:
                    # Retry without cluster information from the beginning of ranking
                    count_non_removed = len(spectrum.failing())
                    warning("flitsr found", count_non_removed,
                            "failing test(s) that it could not explain - retrying without cluster information")
                    # Do NOT reset bucket - keep removed tests and entities removed
                    return self.flitsr(spectrum, formula, use_cluster=False)
                else:
                    count_non_removed = len(spectrum.failing())
                    warning("flitsr found", count_non_removed,
        ...
        faulty = self.flitsr(spectrum, formula, use_cluster=use_cluster)
        self.remove_faulty_elements(spectrum, tests_removed, faulty) #basisの構築部分
        #print("FLITSR identified element", entity, "as faulty")
