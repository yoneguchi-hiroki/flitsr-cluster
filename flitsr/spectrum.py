...
class Spectrum:
    ...
    def _load_test_clusters(self, xls_path: str = None) -> Dict[object, List[str]]:
        """
        Load failing test clusters from the given file.
        If xls_path is None, automatically detect the Math-?? directory from current working directory.
        Returns Dict[cluster_id, list of test names].
        """
        import os
        import re
        
        if xls_path is None:
            # Try to detect Math-?? directory from current working directory
            cwd = os.getcwd()
            match = re.search(r'Math-\d+', cwd)
            if match:
                math_dir = match.group(0)
                xls_path = os.path.join(os.path.dirname(cwd.rsplit(math_dir, 1)[0] + math_dir), 
                                       math_dir, 'ReClues_clustering.xls')
            else:
                raise FileNotFoundError(f"Could not detect Math-?? directory from current working directory: {cwd}")
        
        if not os.path.exists(xls_path):
            raise FileNotFoundError(f"Cluster file not found: {xls_path}")
        
        book = xlrd.open_workbook(xls_path)
        sheet = book.sheet_by_index(0)
        clusters: Dict[object, List[str]] = {}
        for rowx in range(sheet.nrows):
            test_name = str(sheet.cell_value(rowx, 0))
            cluster_id = sheet.cell_value(rowx, 1)
            clusters.setdefault(cluster_id, []).append(test_name)
        return clusters

    def compare_executing_and_cluster(self, executing: Set[Spectrum.Test],
                                      clusters: Dict[object, List[str]]):
        """
        Compare executing test names with cluster test names.
        Returns (matched_cluster_tests, should_remove):
        - If executing_names ⊆ cluster_names for exactly 1 cluster: return that cluster's tests with should_remove=True
        - If executing_names ⊆ cluster_names for 2+ clusters: return combined tests with should_remove=False
        - Otherwise: return empty set with should_remove=False
        """
        executing_names = {t.name for t in executing}
        matched_clusters = []  # Store all matching clusters
        
        for cluster_id, cluster_tests in clusters.items():
            cluster_names = set(cluster_tests)
            # Check if executing set is wholly contained in this cluster
            if executing_names.issubset(cluster_names):
                matched_clusters.append((cluster_id, cluster_tests))

        # Convert cluster test names to Spectrum.Test objects
        name_to_test = {t.name: t for t in self.tests()}
        result = set()
        should_remove = False
        
        if matched_clusters:
            if len(matched_clusters) == 1:
                # Single cluster match: remove tests
                selected_cluster = matched_clusters[0][1]
                should_remove = True
            else:
                # Multiple clusters match: combine all and don't remove
                selected_cluster = []
                for cluster_id, cluster_tests in matched_clusters:
                    selected_cluster.extend(cluster_tests)
                should_remove = False
            
            for test_name in selected_cluster:
                if test_name in name_to_test:
                    result.add(name_to_test[test_name])
        
        return result, should_remove

    def get_tests(self, entity: Spectrum.Entity, only_failing=False,
                  remove=False, bucket='default', add_failing_cluster=False) -> Set[Spectrum.Test]:
        """
        ...
        #executingがflitsrでは削除するべきentityを実行したテストケースの集合
        #executingを失敗テストのクラスタ結果を参考に削除するべきテストを再導入する
        should_remove_test = remove
        if (add_failing_cluster and only_failing):
            cluster = self._load_test_clusters()  # No argument needed - auto-detect path
            executing, should_remove_test = self.compare_executing_and_cluster(executing, cluster)

        #print("Tests executing", entity, ":", executing)
        #print("remove:", remove and should_remove_test)

        if (remove and should_remove_test):
